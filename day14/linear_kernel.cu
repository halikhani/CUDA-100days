...
int main(){
    ...
    checkCudaStatus(allocateDeviceMemory(&device_input, input_size * sizeof(float)));
    checkCudaStatus(allocateDeviceMemory(&device_weights, weights_size * sizeof(float)));
    checkCudaStatus(allocateDeviceMemory(&device_bias, bias_size * sizeof(float)));
    checkCudaStatus(allocateDeviceMemory(&device_output, output_size * sizeof(float)));

    // Copy data to device
    checkCudaStatus(cudaMemcpy(device_input, host_input, input_size * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaStatus(cudaMemcpy(device_weights, host_weights, weights_size * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaStatus(cudaMemcpy(device_bias, host_bias, bias_size * sizeof(float), cudaMemcpyHostToDevice));

    // cuBLAS handle creation
    cublasHandle_t cublas_handle;
    checkCublasStatus(cublasCreate(&cublas_handle));
    ...
    checkCudaStatus(cudaMemcpy(host_output, device_output, output_size * sizeof(float), cudaMemcpyDeviceToHost));

    std::cout << "Linear layer operation completed successfully" << std::endl;
    printMatrix("Output", host_output, batch_size, output_features);
